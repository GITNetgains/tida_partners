...
class ApiProvider {
  ...
  Future<bool> deletePackage(String id, {bool isAcademy=false}) async {
    ...
    data['token'] = token;
    data['id'] = id;
    http.Response res = await http.post(Uri.parse(isAcademy?DELETE_ACADEMY:DELETE_PACKAGE),
        headers: headers, body: data);
  ...
  Future<FacilitySlotResponse?> fetchSlots(Map<String, String> data) async {
    ...
    http.Response res =
    await http.post(Uri.parse(FETCH_VENU_SLOTS), headers: headers, body: data);
    print(jsonEncode(res.body));
    if (res.statusCode == 200) {
      FacilitySlotResponse datares = FacilitySlotResponse.fromJson(jsonDecode(res.body));
      if (datares.status == true) {
        return datares;
      }
    } else {
  ...
  Future<AllOrdersResponse?> fetchOrders() async {
    ...
    AllOrdersResponse datares = AllOrdersResponse.fromJson(jsonDecode(res.body));
